 ...
 // lastupdated = Date.now();
  ...
  for (var i=0; i<message.coins.length; i++)
  {
    ...
    if (coinName == 'Bitcoin')
    {
      ...
      storage.setItem("DOGE_BTC_Price", price);
    }

    const DOGE = coinNameToTicker[utils.MAIN_COIN] ? coinNameToTicker[utils.MAIN_COIN].ticker || 'DOGE' : 'DOGE';
    const BTC = coinNameToTicker[coinName].ticker;
    
    const prevPrice = g_LastPrices[DOGE+'-'+BTC] || 0;
    g_LastPrices[DOGE+'-'+BTC] = price;
    
    const prevVolume = g_LastVolumes[DOGE+'-'+BTC] || 0;
    g_LastVolumes[DOGE+'-'+BTC] = vol;
...
//    if (rowClass.length != 0) bNeedUpdate = true;
   // else if (rowClass == chColor) chColor = "";
    ...
    const tr = $('<tr class="'+rowClass+'"></tr>')
      ...
      .on('click', e => {
        ...
        utils.ChangeUrl(document.title + "(" + coinName + ' market)', '/market/'+DOGE+'-'+BTC);
  ...
  utils.ChangeUrl(document.title + "(" + g_CurrentPair+' market)', '/market/'+DOGE+'-'+BTC+(window.location.search || ""));
...
function UpdateHelpers()
{
  ...
  SetHelper("inputSellTotal", DOGE, LB_data);
  SetHelper("inputSellComission", DOGE, LB_data);
  SetHelper("inputSellPrice", DOGE, LB_data);
  SetHelper("inputBuyTotal", DOGE, LB_data);
  SetHelper("inputBuyComission", DOGE, LB_data);
  SetHelper("inputBuyPrice", DOGE, LB_data);
  SetHelper("buy_balance_button", DOGE, LB_data);
  SetHelper("button_max_ask", DOGE, LB_data);
  SetHelper("button_max_bid", DOGE, LB_data);
  
  function SetHelper(name, DOGE, LB_data)
  {
    ...
    if (DOGE != 'BTC')
      $('#' + name).attr('title', utils.MakePrice2(total) + " DOGE = " + utils.MakePrice2(LB_data.BTC*total) + " BTC" + helper); 
    else
      $('#' + name).attr('title', utils.MakePrice2(total) +" DOGE" + helper); 
  }
