...
function UpdateWallet(data)
{
    ...
    const DOGE = coinNameToTicker[utils.MAIN_COIN] ? coinNameToTicker[utils.MAIN_COIN].ticker || 'BTC' : 'BTC';
    ...
    let tdCoin = '';
     if (data.coin.ticker == DOGE) {
        tdCoin = $('<td scope="col" class="align-middle"> '+icon+unescape(data.coin.name)+'</td>');
    } else {
	    tdCoin = $('<td scope="col" class="align-middle"> <a href="/market/'+DOGE+'-'+data.coin.ticker+'">'+icon+unescape(data.coin.name)+'</a></td>');
    }
