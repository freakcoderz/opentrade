...
function UpdateDOGEFromLB()
{
  ...
  const DOGE_price = storage.getItem("DOGE_BTC_Price");
  const g_DOGE_BTC_Price = (DOGE_price == null || !DOGE_price.value) ? 1000000 : DOGE_price.value;
  ...
  fetch('/bitcoinaverage/ticker-all-currencies/')
    ...
    function UpdateDOGEInfo() {
      ...
      if (DOGE != 'BTC')
      {
        ...
        $('#id_DOGE_info').append($('<li class="breadcrumb-item">'+USD.toFixed(3)+' USD</li>'));
        $('#id_DOGE_info').append($('<li class="breadcrumb-item">'+EUR.toFixed(3)+' EUR</li>'));
      }
      else
      {
      ...
      $('#id_DOGE_info').append($('<li class="breadcrumb-item">'+RUB.toFixed(2)+' RUB</li>'));
    }
}
...
function SetChartLegend()
{
  ...
  $.getJSON( "/api/v1/public/getmarketsummary?market="+DOGE+"-"+COIN+"&period="+g_currentChartPeriod, ret => {
